...
export async function getSummary(userId, dateFilter) {
    if (!userId) {
...
//     return {names}
// }
export async function getCategoryBreakdown(userId, dateFilter) {
    if (!userId)
...
//     return fullBudget;
// }
export async function getBudgetUsage(userId, month, year) {
    if (!userId)
        throw new Error("Unauthorized");
    const startDate = new Date(year, month - 1, 1);
    const endDate = new Date(year, month, 1);
    ...
    const fullBudget = budgets.map((b) => {
